const { getAllFlights, searchFlights, createFlight, updateFlight, deleteFlight } = require('../models/flightModel');
...
const addFlight = async (req, res) => {
  try {
    const { origin, destination, departure_time, price, seats_available, airline } = req.body;
    if (!origin || !destination || !departure_time || !price || !seats_available) {
      return res.status(400).json({ error: 'All fields are required' });
    }

    const flight = await createFlight(origin, destination, departure_time, price, seats_available, airline || 'SkyJet');
